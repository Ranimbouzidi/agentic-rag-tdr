...
# Helpers AMI
# -------------------------
def _extract_between(text: str, start_markers: list[str], end_markers: list[str], max_len: int = 3000) -> str:
    t = text or ""
    low = t.lower()
    ...
    start = min(starts)
    # on coupe à la première fin trouvée après start
    end_pos = None
...
def structure_ami(text: str) -> Dict[str, Any]:
    ...
    contexte = _extract_between(
        full,
        start_markers=["république", "programme", "prêt", "pret", "appel à manifestations", "manifestations d’intérêt", "manifestations d'interet"],
        end_markers=["les services comprennent", "les services incluent", "le ministère", "invite les firmes", "invite les firmes de consultants"],
        max_len=2400,
    )
    ...
    taches = extract_services_list(mission_block or full)

    # livrables AMI = généralement pièces à fournir + références
