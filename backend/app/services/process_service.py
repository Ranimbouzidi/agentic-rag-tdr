...
def process_document(doc_id: str) -> dict:
    ...
    with engine.begin() as conn:
        ...
        local_dir.mkdir(parents=True, exist_ok=True)  # ✅ avoid missing folder issues
        local_raw = local_dir / filename

        t_dl0 = time.perf_counter()
        download_file(raw_bucket, raw_object_key, local_raw)
        log.info(f"[PROCESS] step=download_raw_done doc_id={doc_id} path={local_raw} took={time.perf_counter()-t_dl0:.2f}s")
        ...
        log.info(f"[PROCESS] step=detect_doc_type_done doc_id={doc_id} doc_type={doc_type} took={time.perf_counter()-t_dt0:.2f}s")
        ...
        log.info(f"[PROCESS] step=upload_txt_done doc_id={doc_id} key={txt_key} took={time.perf_counter()-t_up0:.2f}s")
        ...
        if getattr(extracted, "markdown", None) and extracted.markdown.strip():
            ...
            log.info(f"[PROCESS] step=upload_md_done doc_id={doc_id} key={md_key} took={time.perf_counter()-t_md0:.2f}s")
